...
/**
 * @typedef {Object} BrokerUrlValidation
 * @property {boolean} valid - Whether the URL is valid
 * @property {string} [error] - Error message if invalid
 * @property {string} [normalised] - Normalised URL if valid
 */

/**
 * Validate a broker WebSocket URL.
 *
 * @param {string} url - URL to validate
 * @returns {BrokerUrlValidation} Validation result
 *
 * @example
 * const result = validateBrokerUrl('ws://localhost:8765');
 * if (!result.valid) {
 *   showError(result.error);
 * }
 */
export function validateBrokerUrl(url) {
  ...
  if (!trimmed) {
    return { valid: false, error: 'Broker URL cannot be empty' };
  }
  ...
  if (!trimmed.startsWith('ws://') && !trimmed.startsWith('wss://')) {
    return {
      valid: false,
      error: 'Broker URL must start with ws:// or wss://'
    };
  }
  ...
  // Validate protocol again (URL constructor normalises it)
  if (parsed.protocol !== 'ws:' && parsed.protocol !== 'wss:') {
    return {
      valid: false,
      error: 'Broker URL must use WebSocket protocol (ws:// or wss://)'
    };
  }

  // Check for hostname
  if (!parsed.hostname) {
    return { valid: false, error: 'Broker URL must include a hostname' };
  }

  // Normalise: remove trailing slash, default port handling
  let normalised = `${parsed.protocol}//${parsed.hostname}`;
  if (parsed.port) {
    normalised += `:${parsed.port}`;
  }
  if (parsed.pathname && parsed.pathname !== '/') {
    normalised += parsed.pathname;
  }
