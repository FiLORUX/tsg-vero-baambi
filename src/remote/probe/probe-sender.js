...
export class ProbeSender {
  ...
  set brokerUrl(url) {
    const validation = validateBrokerUrl(url);
    if (!validation.valid) {
      console.warn(`[ProbeSender] Invalid broker URL: ${validation.error}`);
      return;
    }

    this.#brokerUrl = validation.normalised;
    this.#saveSetting(STORAGE_KEY_BROKER_URL, validation.normalised);

    // Update client if connected
    if (this.#client) {
      this.#client.setUrl(validation.normalised);
    }
  }
   ...
   * Processes remote commands such as trim adjustments.
   * @private
   */
  #handleMessage(data) {
    let message;
    ...
    switch (message.type) {
      case 'setTrim':
        this.#handleSetTrim(message);
        break;
    }
  }

  /**
   * Handle remote trim command.
   * Applies trim value to appropriate input source via SSOT model.
   *
   * @param {Object} message - Trim command message
   * @param {number} message.trimDb - Trim value in dB
   * @param {string} [message.inputMode] - Target input mode ('browser' | 'external')
   * @private
   */
  #handleSetTrim(message) {
    const { trimDb, inputMode } = message;

    if (typeof trimDb !== 'number' || !isFinite(trimDb)) {
      console.warn('[ProbeSender] Invalid trim value received:', trimDb);
      return;
    }

    // Clamp to safe range
    const clampedTrim = Math.max(-48, Math.min(24, trimDb));

    // Determine which input to adjust
    const targetMode = inputMode || appState.get('inputMode');

    if (targetMode === InputMode.BROWSER || targetMode === 'browser') {
      appState.set({ browserTrim: clampedTrim });
      console.log(`[ProbeSender] Remote trim applied: browser ${clampedTrim.toFixed(1)} dB`);
    } else if (targetMode === InputMode.EXTERNAL || targetMode === 'external') {
      appState.set({ externalTrim: clampedTrim });
      console.log(`[ProbeSender] Remote trim applied: external ${clampedTrim.toFixed(1)} dB`);
    } else {
      console.log('[ProbeSender] Remote trim ignored: generator mode has no trim');
    }
  }
