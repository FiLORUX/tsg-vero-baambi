...
export class MetricsReceiver {
  ...
  set brokerUrl(url) {
    ...
    if (!validation.valid) {
      console.warn(`[MetricsReceiver] Invalid broker URL: ${validation.error}`);
      return;
    }
  ...
  /**
   * Send a trim adjustment command to a remote probe.
   * The probe will apply this trim to its active input source.
   *
   * @param {string} probeId - Target probe identifier
   * @param {number} trimDb - Trim value in dB (-48 to +24)
   * @param {string} [inputMode] - Optional target input mode ('browser' | 'external')
   * @returns {boolean} True if command was sent
   */
  sendTrim(probeId, trimDb, inputMode = null) {
    if (!this.#client?.isConnected) {
      console.warn('[MetricsReceiver] Cannot send trim: not connected');
      return false;
    }

    if (typeof trimDb !== 'number' || !isFinite(trimDb)) {
      console.warn('[MetricsReceiver] Invalid trim value:', trimDb);
      return false;
    }
    ...
    const message = {
      type: 'setTrim',
      probeId,
      trimDb: Math.max(-48, Math.min(24, trimDb))
    };
    ...
    console.log(`[MetricsReceiver] Sent trim command to ${probeId}: ${trimDb.toFixed(1)} dB`);
    return true;
  }
