 ...
 * PURPOSE
 * ───────
 * Analyse stereo signal characteristics for broadcast quality verification:
...
// ─────────────────────────────────────────────────────────────────────────────
// L/R BUFFER SYNCHRONISATION GLITCH DETECTION
// ─────────────────────────────────────────────────────────────────────────────
// Web Audio API's AnalyserNode has no atomic read for stereo channels.
// Sequential getFloatTimeDomainData() calls can receive buffers from different
// audio blocks, causing intermittent decorrelation on perfectly correlated signals.
 ...
 */
const GLITCH_CONFIG = {
  /** Maximum plausible single-frame correlation drop */
  THRESHOLD: 0.15,
  /** Only filter on frames shorter than this (ms) */
  FRAME_LIMIT_MS: 50,
  /** Minimum correlation for glitch detection to apply */
  MIN_PREV_CORRELATION: 0.8
};

/**
 * State for glitch filtering.
 * @type {Object}
 */
const glitchState = {
  prevCorrelation: 0,
...
/**
 * Filter single-frame correlation glitches caused by L/R buffer desync.
 *
 * Detects sudden large drops in correlation on short frames and holds
 * the previous value instead of showing the glitched reading.
 *
 * @param {number} rawCorrelation - Newly calculated correlation
 * @param {number} [timestamp=performance.now()] - Current timestamp
 * @returns {number} Filtered correlation value
 */
export function filterGlitchCorrelation(rawCorrelation, timestamp = performance.now()) {
  const frameDelta = timestamp - glitchState.prevTimestamp;
  const drop = glitchState.prevCorrelation - rawCorrelation;

  // Detect glitch: large sudden drop on a short frame from a high correlation
  const isLikelyGlitch =
    drop > GLITCH_CONFIG.THRESHOLD &&
    frameDelta < GLITCH_CONFIG.FRAME_LIMIT_MS &&
    glitchState.prevCorrelation > GLITCH_CONFIG.MIN_PREV_CORRELATION;

  if (isLikelyGlitch) {
    // Hold previous value - this is likely a timing glitch
    glitchState.glitchCount++;
    glitchState.prevTimestamp = timestamp;
    return glitchState.prevCorrelation;
  }

  // Normal case: update state and return raw value
  glitchState.prevCorrelation = rawCorrelation;
  glitchState.prevTimestamp = timestamp;
  return rawCorrelation;
}

/**
 * Get glitch detection statistics.
 * @returns {{count: number}} Glitch statistics
 */
export function getGlitchStats() {
  return { count: glitchState.glitchCount };
}
...
export function resetGlitchState() {
  glitchState.prevCorrelation = 0;
  glitchState.prevTimestamp = 0;
  glitchState.glitchCount = 0;
}
...
export class StereoMeter {
   ...
   * @param {Float32Array} leftBuffer - Left channel samples
   * @param {Float32Array} rightBuffer - Right channel samples
   * @param {number} [timestamp=performance.now()] - Frame timestamp for glitch detection
   */
  update(leftBuffer, rightBuffer, timestamp = performance.now()) {
    // Calculate raw correlation
    let rawCorrelation = calculateCorrelation(leftBuffer, rightBuffer);
    rawCorrelation = Math.max(-1, Math.min(1, rawCorrelation));

    // Apply glitch filter to catch L/R buffer desync artifacts
    // See docs/STEREO-SAMPLING-ARCHITECTURE.md for details
    this.correlationRaw = filterGlitchCorrelation(rawCorrelation, timestamp);
