...
export class BalanceMeter {
  ...
  /**
   * Draw the balance meter with a pre-calculated balance value.
   * Used for remote metering where balance is computed remotely.
   *
   * @param {number} balance - Pre-calculated balance value in dB (-12 to +12)
   *                           Positive = L louder, Negative = R louder
   */
  drawValue(balance) {
    if (!this.ctx || !this.canvas) return;

    // Clamp and smooth the remote value
    const clampedBalance = Math.max(-12, Math.min(12, balance));
    const a = 0.15;
    this.monoDevHold = isFinite(clampedBalance) ? this.monoDevHold + a * (clampedBalance - this.monoDevHold) : 0;
    ...
    let col;
    const absDev = Math.abs(this.monoDevHold);
    if (absDev < 1.5) col = getCss('--ok');
    else if (absDev < 3) col = getCss('--cyan');
    else if (absDev < 6) col = getCss('--warn');
    else col = getCss('--hot');
