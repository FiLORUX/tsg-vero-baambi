...
- **Tauri mode drew no meters** · the render loop was initialised with an argument shape it does not read, so every frame threw before drawing; the measure loop was then re-initialised with an incompatible set, so every tick threw; the R128 reset stayed disabled; and the status line read `Tauri: [object Object]`. Tauri mode now shares the browser mode's render-loop dependencies, keeps the measure loop's module-level initialisation, enables the reset once capture runs and names the backend.
