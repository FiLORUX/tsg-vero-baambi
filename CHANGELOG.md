...
### Fixed
- **Display buffers across a pause in Firefox** · while nothing upstream plays, Gecko hands an AudioWorklet render quanta without input channels, as the Web Audio specification describes, and the stereo sampler skipped them: its display buffers stood still on the last signal, no snapshot was posted, and the first two snapshots after the pause joined audio from before it to the signal after it. Between two tests of the verification tool this put the previous test's pink noise in front of the PPM test's TEST tone; while the PPM was fed from these buffers and held its reading within 6 dB of the signal, the test read 4.7 to 6.0 PPM in Firefox only. Blink and WebKit deliver zeros instead, so Chromium and Safari never showed it. The worklet now writes such quanta to its display buffers as zeros and keeps posting snapshots, as its measurements already did, so the goniometer, correlation, balance, stereo analysis and RMS readings fall silent with the signal and nothing from before a pause reaches the next signal. `tests/ppm-feed-test.js` section 3 covers the pause.
- **Nordic and BBC PPM return to a lower level** · the quasi-peak detectors held their reading whenever the signal stayed within 6 dB of it, meant to ride out the zero crossings of a sine, which the 5 ms integration window already does. A reading therefore never fell to a steady signal less than 6 dB below it: a tone stepping from −12 to −17 dBFS went on reading −12, and in Firefox, where the verification tool's PPM test began with the previous test's pink noise still in the buffer, the reading stuck 5.3 dB high and the test failed. Below the reading the detectors now return at the IEC 60268-10 rate (20 dB in 1.7 s, 24 dB in 2.8 s for Type IIa), stopping at the window peak, so steady tones still read without ripple (a 40 Hz tone ripples by 0.08 dB). `tests/ppm-feed-test.js` section 7 covers the drop, the transient and the ripple.
- **Sample Peak ballistics independent of the frame rate** · the meter moved its reading a quarter of the way towards each new peak per update, and derived hold, maximum and clip from that smoothed value. Its response therefore followed the frame rate, and short peaks under-read: a single full-scale sample in low-level noise read −12.7 dBFS at 30 fps and −0.4 dBFS at 180 fps, and a clip shorter than a few frames never lit the clip indicator. The bar now takes each peak at once and falls 20 dB in 1.7 s on the meter's own clock, as the True Peak bar does; hold, maximum and the latched clip indication come from the unsmoothed peak. The meter is fed the largest samples since its previous update: from the stereo sampler, which measures every sample, from the engine's packets in Tauri mode, and from the analyser window only without the sampler. The `smoothing` option gives way to `releaseDbPerSecond`, and a `now` clock can be injected. `tests/sample-peak-test.js` checks that 30, 60 and 180 updates per second give the same maximum and clip and bars within one frame of release; `npm run test:browser:ppm` times the displayed return.
- **Remote Sample Peak** · the Sample Peak bar never showed anything in remote mode: the probe sent no sample peak, the receiver's client dropped fields it did not know, and nothing set the values the render loop draws, so the bar sat at −60 dBFS. The probe page now sends the largest sample magnitude since its previous transmission, taken from the stereo sampler, which measures every sample (`consumeSamplePeaks()`; the worklet reports about every 10 ms, the ScriptProcessor fallback per block); the analyser window, which covers 85 of every 100 ms, remains the fallback only. The receiver displays it with a 3 s hold like the other remote bars, `src/remote/types.js` documents the field, and the REST probe detail includes it. `npm run test:browser` section 5 checks the chain end to end.
...
- **PPM return times in local metering** · the render loop fed the Nordic PPM the whole 4096-sample display window on every frame. Its detector advances one sample per input sample, so overlapping windows replayed samples and ran its clock several times fast: the displayed reading fell 20 dB in 0.3 to 0.7 s instead of 1.7 s. The BBC PPM detector never ran at all, because its branch tested `config.sampleRate`, which the render loop's config does not have, and the bar showed the True Peak reading instead (24 dB in 2.0 s instead of 2.8 s). The stereo-sampler AudioWorklet now runs both IEC 60268-10 detectors on every sample and reports their largest readings about every 10 ms (`consumePpm()`, `resetPpm()`); the ScriptProcessor fallback runs them on each block, and without the sampler the application feeds its own detectors only the analyser samples that are new since the previous frame. `QuasiPeakDetector` reads exactly as `calculateQuasiPeakRC()` and `calculateBBCQuasiPeakRC()` with an O(1) window maximum, and `PPMMeter.updateFromReadings()` takes its readings. The Meter Verification Tool and the probe page use the same feed. Verified by `npm run test:browser:ppm`: 20 dB in 1.7 s and 24 dB in 2.8 s as displayed, with and without the AudioWorklet.
- **Tauri Nordic PPM, dBFS and Sample Peak from the engine** · in Tauri mode the three meters measured the display buffers, which are spliced from the engine's 512-sample snapshots: consecutive snapshots overlap or leave gaps, so the PPM's sample-by-sample detector replayed samples and ran its clock fast, the dBFS meter counted overlapped samples twice and gap samples not at all, and a peak that fell into a gap never reached the Sample Peak meter. The Nordic PPM now shows the engine's Type I reading, clamped to the display range as in local metering, with its hold kept per packet. The engine measures sample peak and RMS on every sample and reports them per packet; `LevelWindow` rebuilds from those packets the window the local meters measure (the latest 4096 samples), and `SamplePeakMeter.updateFromPeaks()` applies the same smoothing, hold and clip indication as `update()`. Verified by `npm run test:browser:tauri`, which feeds the application engine levels that differ from its display snapshots.
- **Remote Nordic PPM scale** · the receiver converted the probe's dBFS reading to the PPM scale with +9 instead of +18 (−18 dBFS read −9.0 instead of 0.0) and showed silence as −49.0 instead of −∞, because its silence threshold lay below the bottom of the probe's range. It now uses `dbfsToPPM()` and the silence threshold of local metering.
- **Tauri render loop ran at all** · `initTauriMode()` passed the render loop a dependency set without `helpers`, `config` or `captureState`, so it threw on its first frame and on every metering packet after it: no bar meter was drawn in Tauri mode, and without `captureState` the render loop could not have recognised Tauri mode anyway. Browser and Tauri mode now share one dependency set (`renderLoopDependencies()`). `initTauriMode()` also re-initialised the measure loop with an incomplete set, which made the running loop throw on every tick; it now keeps the loop as initialised at module load.
