...
Validate and normalise a broker WebSocket URL.

```javascript
import { validateBrokerUrl } from './src/remote/index.js';

const result = validateBrokerUrl('ws://localhost:8765');
// → { valid: true, normalized: 'ws://localhost:8765' }

const invalid = validateBrokerUrl('http://localhost');
// → { valid: false, error: 'Broker URL must start with ws:// or wss://' }
```

### ProbeSender

Send metrics to a broker.

```javascript
import { ProbeSender } from './src/remote/index.js';
...
Receive metrics from probes via broker.

```javascript
import { MetricsReceiver } from './src/remote/index.js';

const receiver = new MetricsReceiver({ brokerUrl: 'ws://broker.local:8765' });
...
*Generated for VERO-BAAMBI v2.3.0 (unreleased)*
